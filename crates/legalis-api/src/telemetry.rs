...
pub fn init_telemetry(config: OtelConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Configure tracer
    let mut tracer_builder = opentelemetry_otlp::new_pipeline()
        .tracing()
        .with_trace_config(
            trace::config()
    ...
    if let Some(endpoint) = config.otlp_endpoint {
        tracer_builder = tracer_builder.with_exporter(
    ...
    let tracer = tracer_builder.install_batch(opentelemetry_sdk::runtime::Tokio)?;
