...
pub mod service {
    ...
    pub mod pb {
        ...
        pub const FILE_DESCRIPTOR_SET: &[u8] = include_bytes!("../proto/legalis_descriptor.bin");
    }
        ...
        fn statute_to_proto(statute: &Statute) -> pb::Statute {
            pb::Statute {
                ...
                preconditions: statute
                    ...
                    .collect(),
                discretion_logic: statute.discretion_logic.clone(),
                exceptions: statute
                    .exceptions
                    .iter()
                    .map(|e| pb::Exception {
    ...
    impl LegalisService for LegalisGrpcService {
        async fn list_statutes(
            ...
            let filtered: Vec<_> = if let Some(jurisdiction) = &req.jurisdiction {
                statutes
                    .iter()
        ...
        async fn delete_statute(
            ...
            Ok(Response::new(DeleteStatuteResponse {
                success: true,
                message: Some(format!("Statute {} deleted successfully", req.id)),
            }))
        }
        ...
        async fn batch_create_statutes(
            ...
            for create_req in req.statutes {
                ...
                if let Some(jurisdiction) = create_req.jurisdiction {
                    statute = statute.with_jurisdiction(jurisdiction);
                }

                if let Some(version) = create_req.version {
                    statute = statute.with_version(version);
                }
        ...
        async fn search_statutes(
            ...
            let filtered: Vec<_> = statutes
                ...
                .filter(|s| {
                    ...
                    let jurisdiction_match = req
                        .jurisdiction
                        .as_ref()
                        .map(|j| s.jurisdiction.as_ref() == Some(j))
                        .unwrap_or(true);
