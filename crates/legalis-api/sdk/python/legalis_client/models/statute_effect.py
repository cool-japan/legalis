...
class StatuteEffect(BaseModel):
    """
    Legal effect (Then clause)
    """ # noqa: E501
    description: StrictStr
    effect_type: StrictStr
    parameters: Optional[Dict[str, StrictStr]] = None
    __properties: ClassVar[List[str]] = ["description", "effect_type", "parameters"]

    @field_validator('effect_type')
    def effect_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Grant', 'Revoke', 'Obligation', 'Prohibition', 'MonetaryTransfer', 'StatusChange']):
            raise ValueError("must be one of enum values ('Grant', 'Revoke', 'Obligation', 'Prohibition', 'MonetaryTransfer', 'StatusChange')")
        return value
