...
impl SsoManager {
    ...
    pub fn handle_callback(&self, provider: SsoProvider, code: String) -> Result<SsoUser, String> {
        if code.is_empty() {
...
impl FieldEncryptionManager {
    ...
    pub fn encrypt_field(&self, field_name: &str, value: &str) -> Result<EncryptedField, String> {
        if !self
            .config
            .encrypted_fields
...
mod tests {
    ...
    fn test_ldap_user_group_membership() {
        let mut user = LdapUser::new("uid=jdoe,dc=example,dc=com".to_string(), "jdoe".to_string());
    ...
    fn test_field_encryption_config_creation() {
        let config = FieldEncryptionConfig::new("AES-256-GCM".to_string(), "key123".to_string())
