...
//!
//! Hash-based signatures rely only on the second-pre-image / collision resistance
//! of a hash function, properties that (unlike RSA and elliptic curves) are
//! **not** broken by Shor's algorithm. They are the most conservative family of
//! post-quantum signatures and the implemented PQ option for the registry.
//!
...
//!    public elements. Each signature carries the revealed secret element and the
//!    sibling public element for every message-digest bit, which lets a verifier
//!    reconstruct the fingerprint and compare it to the trusted one.
...
//!
//! This is a real, correct hash-based OTS suitable for long-term archival
//! integrity. It is **not** a standardized, audited scheme (it omits the WOTS+
//! few-time optimisations of XMSS / SPHINCS+); standardized lattice and stateless
...
/// A Lamport one-time signature: for each digest bit, the revealed secret element
/// and the sibling public element.
...
/// Reconstructs the public-key fingerprint implied by `signature` over `message`.
///
/// A correct fingerprint can only be produced with knowledge of the secret
/// pre-images, so comparing it to a trusted fingerprint verifies the signature.
///
/// # Errors
///
/// Returns [`RegistryError::InvalidOperation`] if the signature is malformed.
...
/// The long-lived public key of a Merkle signature scheme: the tree root and its
/// height.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
impl MerklePublicKey {
    ...
    /// # Errors
    ///
    /// Returns [`RegistryError::InvalidOperation`] if the root is not valid hex.
    pub fn root_bytes(&self) -> RegistryResult<[u8; 32]> {
...
impl MerkleSigner {
    ...
    pub fn from_seed(master_seed: [u8; 32], height: u8) -> RegistryResult<Self> {
    ...
    /// # Errors
    ///
    /// Returns [`RegistryError::InvalidOperation`] if `index` is out of range or
    /// has already been used (hash-based one-time keys must not be reused).
    pub fn sign(&mut self, index: u32, message: &[u8]) -> RegistryResult<MerkleSignature> {
...
mod tests {
    ...
    fn test_lamport_sign_verify_roundtrip() {
        ...
        let signature = keypair.sign(b"sign this statute");
        ...
        assert_eq!(keypair.public_fingerprint_hex().len(), 64);
    }
    ...
    fn test_lamport_is_deterministic() {
        ...
        assert_eq!(seed_from_bytes(b"pass"), seed_from_bytes(b"pass"));
        assert_ne!(seed_from_bytes(b"pass"), seed_from_bytes(b"other"));
    }
    ...
    fn test_merkle_sign_verify_multiple_leaves() {
        ...
        assert_eq!(signer.height(), 3);
        let public_key = signer.public_key();
        for index in [0u32, 3, 7] {
            let message = format!("statute revision {index}");
