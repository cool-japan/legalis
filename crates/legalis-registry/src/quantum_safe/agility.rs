//! Cryptographic agility: a catalogue of post-quantum algorithms and a suite
//! selector for registry records.
//!
//! "Cryptographic agility" is the ability to change the primitives protecting an
//! artifact without redesigning the artifact. For long-lived statutes this lets
//! the registry answer: "is this algorithm still safe?", "what should I migrate
//! to?" and "which post-quantum algorithms are planned but not yet bundled?".
//!
//! Standardized lattice schemes (ML-DSA / ML-KEM, FIPS 203/204) and stateless
//! hash-based SLH-DSA (FIPS 205) are registered with [`AlgorithmStatus::Planned`]
//! and intentionally **not** implemented, to avoid pulling in heavy,
//! non-pure-Rust dependencies; the implemented post-quantum signature is the
//! hash-based scheme in [`super::hash_sig`].
...
pub enum AlgorithmStatus {
    /// Preferred for new records.
    Recommended,
    /// Safe to verify and acceptable for new records, but not preferred.
    Acceptable,
    /// Must be migrated away from; do not use for new records.
    Deprecated,
    /// Known and reserved, but not yet implemented in this crate.
    Planned,
    /// Implemented but experimental; not for production use.
    Experimental,
}
...
pub enum SignatureScheme {
    ...
    /// ML-DSA-65 (CRYSTALS-Dilithium), FIPS 204. Planned; deferred.
    MlDsa65,
    /// SLH-DSA-SHA2-128s (SPHINCS+), FIPS 205. Planned; deferred.
    SlhDsaSha2128s,
}
...
pub struct PqAlgorithmRegistry {
    entries: BTreeMap<String, AlgorithmDescriptor>,
}

impl PqAlgorithmRegistry {
    ...
    pub fn with_defaults() -> Self {
        ...
        let digests = [
            ...
            (
                QuantumHashAlgorithm::Sha512Trunc256,
                "SHA-512/256",
                256,
                128,
                AlgorithmStatus::Acceptable,
            ...
            (
                QuantumHashAlgorithm::IteratedSha512 { rounds: 1 },
                "Iterated SHA-512",
                512,
                256,
                AlgorithmStatus::Recommended,
        ...
        // detect and migrate away from it (metadata only; never computed here).
        registry.register(AlgorithmDescriptor {
            id: "sha-1".to_string(),
            kind: AlgorithmKind::Digest,
        ...
        registry.register(AlgorithmDescriptor {
            ...
            quantum_resistant: false,
            status: AlgorithmStatus::Deprecated,
            note: "Elliptic-curve signature; broken by Shor. Migrate to a PQ scheme.".to_string(),
        });
        registry.register(AlgorithmDescriptor {
            ...
            quantum_resistant: true,
            status: AlgorithmStatus::Planned,
            note: "FIPS 203 key-encapsulation mechanism; deferred (modelled in qkd).".to_string(),
        });
        registry
    }
    ...
    /// Suggests a migration target for `id`: if it is unknown, deprecated, or not
    /// quantum-resistant, returns the recommended algorithm of its kind. Returns
    /// `None` when no migration is needed.
    #[must_use]
...
/// A named bundle of a content-digest algorithm and an optional signature scheme
/// that protects a registry record.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
impl CryptoSuite {
    ...
    pub fn quantum_safe_v1() -> Self {
        Self {
            name: "legalis-registry-pq-v1".to_string(),
            digest: QuantumHashAlgorithm::ConcatSha512Sha256
                .family_id()
                .to_string(),
            signature: Some(
    ...
    /// migration; both the digest and signature scheme are obsolete.
    #[must_use]
    pub fn classical_v1() -> Self {
        Self {
            name: "legalis-registry-classical-v1".to_string(),
            digest: "sha-1".to_string(),
            signature: Some("ed25519".to_string()),
        }
    }

    /// Whether every algorithm named by the suite is registered and
    /// quantum-resistant.
    #[must_use]
    pub fn is_quantum_resistant(&self, registry: &PqAlgorithmRegistry) -> bool {
        if !registry.is_quantum_resistant(&self.digest) {
            return false;
        }
    ...
    /// Returns descriptions of any weak or unknown algorithms the suite names.
    #[must_use]
    pub fn weaknesses(&self, registry: &PqAlgorithmRegistry) -> Vec<String> {
        let mut issues = Vec::new();
        match registry.get(&self.digest) {
            None => issues.push(format!("unknown digest algorithm '{}'", self.digest)),
            Some(descriptor) if descriptor.needs_migration() => {
                issues.push(format!("digest '{}' should be migrated", self.digest));
            }
            Some(_) => {}
        }
        if let Some(scheme) = &self.signature {
    ...
    /// Produces an upgraded suite: any weak digest/signature is replaced by the
    /// registry's recommended algorithm of the same kind.
    #[must_use]
    pub fn upgraded(&self, registry: &PqAlgorithmRegistry) -> CryptoSuite {
        let mut next = self.clone();
        if registry.migration_target(&self.digest).is_some()
            && let Some(recommended) = registry.recommended(AlgorithmKind::Digest)
        {
            next.digest = recommended.id.clone();
        }
        if let Some(scheme) = self.signature.clone()
            && registry.migration_target(&scheme).is_some()
            && let Some(recommended) = registry.recommended(AlgorithmKind::Signature)
        {
            next.signature = Some(recommended.id.clone());
        }
        next
    }
}
...
mod tests {
    ...
    fn test_registry_defaults_present() {
        let registry = PqAlgorithmRegistry::with_defaults();
    ...
    fn test_planned_schemes_listed_but_not_implemented() {
        ...
        assert_eq!(SignatureScheme::from_id("nope"), None);
    }

    #[test]
    fn test_suite_quantum_resistance_and_upgrade() {
        let registry = PqAlgorithmRegistry::with_defaults();
        let pq = CryptoSuite::quantum_safe_v1();
        assert!(pq.is_quantum_resistant(&registry));
        assert!(pq.weaknesses(&registry).is_empty());

        let classical = CryptoSuite::classical_v1();
        ...
        assert_eq!(
            upgraded.signature.as_deref(),
            Some("hash-merkle-lamport-sha256")
        );
        // An already-strong suite is left intact.
        let strong = pq.upgraded(&registry);
        assert_eq!(strong.digest, pq.digest);
        assert_eq!(strong.signature, pq.signature);
    }

    #[test]
    fn test_custom_registration_and_serde() {
        let mut registry = PqAlgorithmRegistry::new();
        assert!(registry.is_empty());
        registry.register(AlgorithmDescriptor {
        ...
        let suite = CryptoSuite::quantum_safe_v1();
        let json = serde_json::to_string(&suite).expect("ser");
        let back: CryptoSuite = serde_json::from_str(&json).expect("de");
