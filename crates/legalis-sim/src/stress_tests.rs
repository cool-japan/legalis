...
mod tests {
    ...
    fn stress_test_checkpoint_manager_capacity() {
        ...
        // Should only keep 100 checkpoints (eviction happens)
        assert_eq!(manager.count(), 100);

        // Count how many checkpoints are available
        // Note: Due to identical timestamps when created in rapid succession,
        // eviction order may not be strictly FIFO. We verify the manager
        // correctly limits capacity rather than specific ID availability.
        let available_count = (0..200)
            .filter(|i| manager.load(&format!("cp-{}", i)).is_some())
            .count();
        assert_eq!(available_count, 100);

        // The most recent checkpoint should always be available
        assert!(manager.load("cp-199").is_some());
    }
