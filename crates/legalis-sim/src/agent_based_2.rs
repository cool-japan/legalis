        ...
        for agent_id in &agent_ids {
            ...
            if agent_ids.len() > 1 {
                let neighbor_id = loop {
                    let idx = rng.gen_range(0..agent_ids.len());
                ...
                {
                    if !neighbor.memes.is_empty() && rng.gen_range(0.0..1.0) < agent.openness {
                        let meme_idx = rng.gen_range(0..neighbor.memes.len());
                        ...
                        if rng.gen_range(0.0..1.0) < acceptance_prob {
                            // Mutation
                            let mut meme = transmitted_meme;
                            if rng.gen_range(0.0..1.0) < meme.mutation_rate {
                                meme.id = Uuid::new_v4();
                                meme.fitness += rng.gen_range(-0.1..0.1);
