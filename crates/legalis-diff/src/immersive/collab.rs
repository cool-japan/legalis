...
//!
//! This is deliberately distinct from [`crate::collaborative_review`] (which
//! manages comments, votes and approval workflows). Here the shared state is the
//! **view** itself: a synchronised camera, per-participant 3-D cursors, scene
//! annotations and a sequence-ordered event log. Concurrent updates are
//! reconciled with last-writer-wins using monotonic per-resource sequence
//! numbers, so two replicas that accept the same operations converge to an
//! identical [`VizSession::state_digest`].
...
impl Default for CameraState {
    fn default() -> Self {
        Self::new(Vec3::new(0.0, 0.0, 8.0), Vec3::zero())
    }
}
...
impl VizEvent {
    ...
    pub fn sequence(&self) -> u64 {
        match self {
            VizEvent::Joined { seq, .. }
            | VizEvent::Left { seq, .. }
            | VizEvent::PresenterSet { seq, .. }
            | VizEvent::CameraMoved { seq, .. }
            | VizEvent::CursorUpdated { seq, .. }
            | VizEvent::AnnotationAdded { seq, .. }
            | VizEvent::AnnotationRemoved { seq, .. }
            | VizEvent::FollowToggled { seq, .. } => *seq,
        }
    }
}
...
pub struct VizSnapshot {
    ...
    pub participants: Vec<VizParticipant>,
...
/// A live collaborative visualization session.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct VizSession {
    session_id: String,
    participants: BTreeMap<String, VizParticipant>,
...
impl VizSession {
    ...
    /// Returns `true` if `participant` is connected.
    fn require_participant(&self, participant: &str) -> DiffResult<()> {
        if self.participants.contains_key(participant) {
            Ok(())
        } else {
            Err(DiffError::Visualization(format!(
    ...
    /// # Errors
    ///
    /// Returns [`DiffError::Visualization`] if the id is already present.
    pub fn join(&mut self, participant: VizParticipant) -> DiffResult<()> {
    ...
    /// Removes a participant and their cursor. Clears the presenter slot if it
    /// was them. Returns `true` if a participant was removed.
    ...
    /// # Errors
    ///
    /// Returns [`DiffError::Visualization`] if `participant` is not connected.
    pub fn set_presenter(&mut self, participant: &str) -> DiffResult<()> {
        self.require_participant(participant)?;
        self.presenter = Some(participant.to_string());
        let seq = self.bump();
        self.log.push(VizEvent::PresenterSet {
            seq,
            participant: participant.to_string(),
        });
        Ok(())
    ...
    /// # Errors
    ///
    /// Returns [`DiffError::Visualization`] if `client_seq` is not newer than
    /// the last accepted toggle (a stale/conflicting update).
    pub fn set_follow(&mut self, enabled: bool, client_seq: u64) -> DiffResult<()> {
        ...
        let seq = self.bump();
        self.log.push(VizEvent::FollowToggled { seq, enabled });
        Ok(())
    }

    /// Moves the shared camera.
    ///
    /// When follow-presenter is on and a presenter is set, only the presenter
    /// may move the camera. Updates are last-writer-wins on `client_seq`.
    ///
    /// # Errors
    ///
    /// Returns [`DiffError::Visualization`] if the actor is unknown, lacks
    /// permission, or `client_seq` is stale.
    pub fn move_camera(
        &mut self,
        actor: &str,
        camera: CameraState,
        ...
        let seq = self.bump();
        self.log.push(VizEvent::CameraMoved {
            seq,
    ...
    /// # Errors
    ///
    /// Returns [`DiffError::Visualization`] if the actor is unknown or the
    /// cursor sequence is stale.
    pub fn update_cursor(&mut self, actor: &str, cursor: PresenceCursor) -> DiffResult<()> {
        self.require_participant(actor)?;
        ...
        let seq = self.bump();
        self.log.push(VizEvent::CursorUpdated {
            seq,
            actor: actor.to_string(),
        });
        Ok(())
    ...
    /// # Errors
    ///
    /// Returns [`DiffError::Visualization`] if the author is unknown or the
    /// annotation id already exists.
    pub fn add_annotation(&mut self, annotation: SceneAnnotation) -> DiffResult<()> {
        ...
        let seq = self.bump();
        self.log.push(VizEvent::AnnotationAdded { seq, id });
        Ok(())
    }
    ...
    pub fn remove_annotation(&mut self, id: &str) -> bool {
        ...
        let seq = self.bump();
        self.log.push(VizEvent::AnnotationRemoved {
            seq,
            id: id.to_string(),
        });
        true
    ...
    /// A deterministic SHA-256 digest of the canonical state snapshot.
    ///
    ...
    /// # Errors
    ///
    /// Returns [`DiffError::SerializationError`] if the snapshot cannot be
    /// serialised.
    pub fn state_digest(&self) -> DiffResult<String> {
...
mod tests {
    ...
    fn session_with_two() -> VizSession {
        let mut s = VizSession::new("sess-1");
        s.join(VizParticipant::new("alice", "Alice").with_role(ParticipantRole::Presenter))
            .expect("alice joins");
        s.join(VizParticipant::new("bob", "Bob"))
            .expect("bob joins");
        s
    }
    ...
    fn test_duplicate_join_rejected() {
        let mut s = session_with_two();
        assert!(s.join(VizParticipant::new("alice", "Alice2")).is_err());
    }

    #[test]
    fn test_camera_move_last_writer_wins() {
        let mut s = session_with_two();
        let cam = CameraState::new(Vec3::new(1.0, 1.0, 1.0), Vec3::zero());
        s.move_camera("alice", cam, 5).expect("first move");
        // Stale (<= current seq) rejected.
        assert!(s.move_camera("alice", cam, 5).is_err());
        assert!(s.move_camera("alice", cam, 3).is_err());
        // Newer accepted.
        let cam2 = CameraState::new(Vec3::new(2.0, 2.0, 2.0), Vec3::zero());
    ...
    fn test_follow_presenter_restricts_camera() {
        ...
        let cam = CameraState::new(Vec3::new(1.0, 0.0, 0.0), Vec3::zero());
    ...
    fn test_unknown_actor_rejected() {
        ...
        assert!(s.move_camera("mallory", cam, 1).is_err());
        assert!(
    ...
    #[test]
    fn test_cursor_updates_and_staleness() {
        let mut s = session_with_two();
        ...
        s.update_cursor(
            "bob",
            PresenceCursor::new(Vec3::new(2.0, 0.0, 0.0), 3).with_focus("law-1"),
        )
        .expect("newer cursor");
        assert_eq!(
            s.cursor("bob").and_then(|c| c.focused_node.clone()),
            Some("law-1".to_string())
        );
    }

    #[test]
    fn test_annotations_add_remove_dedupe() {
        let mut s = session_with_two();
        let ann = SceneAnnotation::new("a1", "bob", Vec3::zero(), "note").on_node("law-1");
    ...
    fn test_event_log_grows_with_sequence() {
        ...
        assert_eq!(log.len(), 3);
        let seqs: Vec<u64> = log.iter().map(VizEvent::sequence).collect();
    ...
    fn test_replicas_converge_after_conflict() {
        ...
        let cam_lo = CameraState::new(Vec3::new(1.0, 0.0, 0.0), Vec3::zero());
        let cam_hi = CameraState::new(Vec3::new(9.0, 0.0, 0.0), Vec3::zero());

        // Replica A: apply winner first, then stale (rejected).
        a.move_camera("alice", cam_hi, 10).expect("hi");
        assert!(a.move_camera("alice", cam_lo, 5).is_err());

        // Replica B: apply stale first (accepted), then winner (overwrites).
    ...
    fn test_snapshot_is_sorted() {
        let mut s = VizSession::new("s");
        s.join(VizParticipant::new("zoe", "Zoe")).unwrap();
        s.join(VizParticipant::new("amy", "Amy")).unwrap();
        let snap = s.snapshot();
