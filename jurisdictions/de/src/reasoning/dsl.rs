//! `legalis-dsl` integration for German law (deutsches Recht).
//!
//! Renders the statutes modelled in [`crate::reasoning::statute_adapter`] as
//! `legalis-dsl` source text, so the labour-law rules can be inspected,
...
/// Renders every modelled German labour statute as `legalis-dsl` source text.
///
...
mod tests {
    ...
    fn statutes_render_as_valid_dsl() {
        ...
        assert!(!statutes.is_empty(), "DE must model at least one statute");
